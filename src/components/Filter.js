import React, { useState } from "react";
import App from "../App";
import { Link } from "react-router-dom";
import Loading from "./Loading";
import { ErrorPage } from "./404Error";
import { Modal, ModalManager, Effect } from "react-dynamic-modal";
import MyModal from "./MyModal";

export default (props) => {
  let randomString,
    sessionCount = 1;
  // let count = 1;
  let baseData = props.data;
  let sessionData = props.sessions;
  let sessionToMember = props.sessionToMember;
  let modifySessionMemberList = props.modifySessionMemberList;
  // console.log("member", props.match);
  let facId = props.match.params.facId;
  let isRecordPresent = false;
  const [modalIsOpen, setIsOpen] = React.useState(false);
  let [currentMemId, setCurrentMemId] = useState();
  //let [isChecked, setIsChecked] = useState(true);
  // const [tempData, setTempData] = useState([]);
  const [editedData, setEditedData] = useState({
    sortByNameID: "",
    sortByFacilitatorID: "",
    sortByGroupID: ""
  });

  let count = 1;

  const goBack = () => {
    props.history.goBack();
  };

  const handleForm = (e) => {
    // console.log(e.target);

    switch (e.target.id) {
      case "inputName":
        setEditedData({
          ...editedData,
          sortByNameID: e.target.value
        });
        break;

      case "inputFac":
        setEditedData({
          ...editedData,
          sortByFacilitatorID: e.target.value
        });
        break;
      case "inputGroup":
        setEditedData({
          ...editedData,
          sortByGroupID: e.target.value
        });
        break;
      // case "inputLName":
      //   setEditedData({ ...editedData, lastName: e.target.value });
      //   break;

      // case "inputGroup":
      //   // console.log(e.target.value);
      //   setEditedData({ ...editedData, group: e.target.value });
      //   break;

      default:
        break;
    }
  };

  const getFacilitator = (facilId) => {
    const temp = props.facilitator.filter((f) => f._id === facilId);
    return temp[0];
  };

  const renderBody = () => {
    if (
      editedData.sortByNameID !== "" &&
      editedData.sortByFacilitatorID === "" &&
      editedData.sortByGroupID === ""
    ) {
      const temp = baseData.filter((d) => d._id === editedData.sortByNameID);
      // console.log("temp", temp);
      return template02(temp);
    }
    if (
      editedData.sortByNameID === "" &&
      editedData.sortByFacilitatorID !== "" &&
      editedData.sortByGroupID === ""
    ) {
      const temp = baseData.filter(
        (d) => d.facilitator === editedData.sortByFacilitatorID
      );
      return template02(temp);
    }
    if (
      editedData.sortByNameID === "" &&
      editedData.sortByFacilitatorID === "" &&
      editedData.sortByGroupID !== ""
    ) {
      const temp = baseData.filter((d) => d.group === editedData.sortByGroupID);
      return template02(temp);
    }
    if (
      editedData.sortByNameID !== "" &&
      editedData.sortByFacilitatorID !== "" &&
      editedData.sortByGroupID === ""
    ) {
      const temp = baseData.filter(
        (d) =>
          d._id === editedData.sortByNameID &&
          d.facilitator === editedData.sortByFacilitatorID
      );
      return template02(temp);
    }

    if (
      editedData.sortByNameID !== "" &&
      editedData.sortByFacilitatorID === "" &&
      editedData.sortByGroupID !== ""
    ) {
      const temp = baseData.filter(
        (d) =>
          d._id === editedData.sortByNameID &&
          d.group === editedData.sortByGroupID
      );
      return template02(temp);
    }

    if (
      editedData.sortByNameID === "" &&
      editedData.sortByFacilitatorID !== "" &&
      editedData.sortByGroupID !== ""
    ) {
      const temp = baseData.filter(
        (d) =>
          d.facilitator === editedData.sortByFacilitatorID &&
          d.group === editedData.sortByGroupID
      );
      return template02(temp);
    }

    if (
      editedData.sortByNameID !== "" &&
      editedData.sortByFacilitatorID !== "" &&
      editedData.sortByGroupID !== ""
    ) {
      const temp = baseData.filter(
        (d) =>
          d._id === editedData.sortByNameID &&
          d.group === editedData.sortByGroupID &&
          d.facilitator === editedData.sortByFacilitatorID
      );
      return template02(temp);
    }

    return template02();
  };

  const template02 = (f1Data = baseData) => {
    // console.log("ff", f1Data);
    if (f1Data.length) {
      return (
        f1Data.length &&
        f1Data.map((d) => {
          // filter
          // if (editedData.sortByNameID) {

          let fac = getFacilitator(d.facilitator);

          return (
            <tr key={d._id}>
              <th scope="row">{count++}</th>
              <td>{d.firstName}</td>
              <td>{fac ? fac.name : "Not Assigned"}</td>
              <td>{d.email}</td>
              <td>{d.mobile}</td>
              <td>{d.remark}</td>

              <td>
                <Link to={`/edit/${d._id}/${d.facilitator}`}>
                  <>
                    {" "}
                    <img
                      src="https://cdn0.iconfinder.com/data/icons/glyphpack/19/edit-512.png"
                      height="20px"
                      widht="20px"
                      alt="edit"
                    />
                  </>
                </Link>
              </td>
              <td>
                {/* <button
                  className="btn btn-danger"
                  onClick={() => props.removeMem(d._id)}
                >
                  Remove
                </button> */}

                <img
                  class="card-img-top"
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQp6HnkckDg_Knd1Ip0jiQdvDkQvPjKpIJJWw&usqp=CAU"
                  alt="Delete icon"
                  style={{
                    float: "right",
                    width: "30px",
                    height: "30px",
                    cursor: "pointer"
                  }}
                  onClick={() => {
                    if (
                      confirm(
                        "Are you sure? \n This will delete me from your group."
                      ) === true
                    ) {
                      props.removeMem(d._id);
                    }
                  }}
                />
              </td>
            </tr>
          );
        })
      );
    } else {
      return <h3>Not Found!</h3>;
    }
  };

  const template = (
    <div>
      <img
        class="card-img-top"
        src="data:image/png;base64,"
        alt=" Back "
        style={{
          float: "right",
          width: "50px",
          height: "50px",
          cursor: "pointer"
        }}
        onClick={goBack}
      />
      {/* {console.log("From", props)} */}
      <h4> Filter</h4>
      <Link to={`/add/${facId}`}>
        <button className="btn btn-primary">Add Member</button>
      </Link>
      <br />
      <br />
      {/* // Filter Form starts */}
      <form>
        <div class="row">
          <div class="column">
            Search by Name:
            <select
              class="form-control"
              id="inputName"
              value={editedData.sortByName}
              onChange={handleForm}
            >
              <option selected value="">
                --Select --{" "}
              </option>
              {baseData.length &&
                baseData.map((d) => {
                  return <option value={d._id}>{d.firstName}</option>;
                })}
            </select>
          </div>

          <span>....</span>
          <div class="column">
            Search by Facilitator:
            <select
              class="form-control"
              id="inputFac"
              value={editedData.sortByFacilitator}
              onChange={handleForm}
            >
              <option selected value="">
                --Select --{" "}
              </option>
              {props.facilitator.length &&
                props.facilitator.map((f) => {
                  return <option value={f._id}>{f.name}</option>;
                })}
            </select>
          </div>
          <span> ....</span>
          <div class="column">
            Search by Group:
            <select
              class="form-control"
              id="inputGroup"
              value={editedData.sortByGroupID}
              onChange={handleForm}
            >
              <option selected value="">
                --Select --{" "}
              </option>
              {props.group.length &&
                props.group.map((f) => {
                  return <option value={f._id}>{f.name}</option>;
                })}
            </select>
            <br />
          </div>
        </div>
      </form>
      {/* // Filter Form ends */}
      <table class="table table-striped">
        <thead class="thead-dark">
          <tr>
            <th scope="col"></th>
            <th scope="col">Name</th>
            <th scope="col">Facilitator</th>
            <th scope="col">Email</th>
            <th scope="col">Mobile</th>
            <th scope="col">Remark</th>
            <th scope="col"></th>
            <th scope="col"></th>
          </tr>
        </thead>
        <tbody>{renderBody()}</tbody>
      </table>
      <br />
      <br />
    </div>
  );
  return (
    <div>
      {!props.inProgress ? template : <Loading />}{" "}
      {/* {console.log("from detail", props.inProgress)} */}
    </div>
  );
};
